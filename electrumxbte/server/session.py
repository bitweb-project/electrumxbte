...
from electrumxbte.lib.hash import (HASHX_LEN, Base58Error, hash_to_hex_str,
                                hex_str_to_hash, sha256)
from electrumxbte.lib.merkle import MerkleCache
from electrumxbte.lib.text import sessions_lines
from electrumxbte.server.daemon import DaemonError
from electrumxbte.server.peers import PeerManager

if TYPE_CHECKING:
    from electrumxbte.server.db import DB
    from electrumxbte.server.env import Env
    from electrumxbte.server.block_processor import BlockProcessor
    from electrumxbte.server.daemon import Daemon
    from electrumxbte.server.mempool import MemPool
...
class SessionManager:
    ...
    def __init__(
        ...
        self._sslc = None
        # Event triggered when electrumxbte is listening for incoming requests.
    ...
    async def rpc_add_peer(self, real_name):
        ...
        real_name: "bch.electrumxbte.cash t50001 s50002" for example
        '''
...
class SessionBase(RPCSession):
    '''Base class of ElectrumXBTE JSON sessions.
    ...
    async def handle_request(self, request):
        '''Handle an incoming request.  ElectrumXBTE doesn't receive
...
class ElectrumXBTE(SessionBase):
    '''A TCP server that handles incoming ElectrumBte connections.'''
    ...
    def server_features(cls, env):
        ...
        return {
            'hosts': hosts_dict,
            'pruning': None,
            'server_version': electrumxbte.version,
    ...
    def server_version_args(cls):
        ...
        return [electrumxbte.version, cls.protocol_min_max_strings()]
    ...
    async def replaced_banner(self, banner):
        ...
        for pair in [
                ('$SERVER_VERSION', electrumxbte.version_short),
                ('$SERVER_SUBVERSION', electrumxbte.version),
    ...
    async def banner(self):
        ...
        banner = f'You are connected to an {electrumxbte.version} server.'
        self.bump_cost(0.5)
    ...
    async def server_version(self, client_name='', protocol_version=None):
        ...
        return electrumxbte.version, self.protocol_version_string()
...
class DashElectrumXBTE(ElectrumXBTE):
    '''A TCP server that handles incoming ElectrumBte Dash connections.'''
...
class SmartCashElectrumXBTE(DashElectrumXBTE):
    '''A TCP server that handles incoming ElectrumBte-SMART connections.'''
...
class AuxPoWElectrumXBTE(ElectrumXBTE):
    async def block_header(self, height, cp_height=0):
