...
ElectrumXBTE takes no command line arguments, instead its behaviour is controlled by
...
.. envvar:: DAEMON_URL
  ...
  ElectrumXBTE will initially connect to the first, and failover to
...
.. envvar:: ELECTRUMXBTE
  ...
  The path to the electrumxbte_server script.  Relative paths should
...
.. note:: ElectrumXBTE will not serve any incoming connections until it has fully caught up
...
.. envvar:: SERVICES
  ...
  A comma-separated list of services ElectrumXBTE will accept incoming connections for.
  ...
  ElectrumXBTE will not listen for any incoming connections.
  ...
  *port* can only be defaulted for **rpc** where the default is :const:`8000`.
  ...
  libcap2-bin package::
    ...
    sudo setcap cap_net_bind_service=+ep /path/to/electrumxbte_server
...
.. envvar:: REPORT_SERVICES
  ...
  A comma-separated list of services ElectrumXBTE will advertize and other servers in the
...
.. envvar:: DONATION_ADDRESS
  ...
  The server donation address reported to ElectrumBte clients.  Defaults
  to empty, which ElectrumBte interprets as meaning there is none.

.. envvar:: BANNER_FILE

  The path to a banner file to serve to clients in ElectrumBte's
  ...
  + ``$SERVER_VERSION`` is replaced with the ElectrumXBTE version you are
    running, such as ``1.0.10``.
  + ``$SERVER_SUBVERSION`` is replaced with the ElectrumXBTE user agent
    string.  For example, ``ElectrumXBTE 1.0.10``.
  ...
  See `here <https://github.com/shsmith/electrumxbte-banner-updater>`_
...
.. envvar:: REORG_LIMIT
  ...
  reorganisation.  ElectrumXBTE retains some fairly compact undo
...
Address subscriptions in ElectrumXBTE are very cheap - they consume about
...
.. envvar:: MAX_SEND
  ...
  because standard ElectrumBte protocol header "chunk" requests are almost
  that large.

  The ElectrumBte protocol has a flaw in that address histories must be
...
.. envvar:: SESSION_GROUP_BY_SUBNET_IPV6
  ...
  If a session disconnects, ElectrumXBTE continues to associate its cost with its IP address,
...
In response to the :func:`server.peers.subscribe` RPC call, ElectrumXBTE
will only return peer servers that it has recently connected to and
verified basic functionality.

If you are not running a Tor proxy ElectrumXBTE will be unable to connect
...
to be running a Tor proxy for ElectrumXBTE to use.

ElectrumXBTE will perform peer-discovery by default and announce itself
...
.. envvar:: PEER_DISCOVERY
  ...
  If ``on``, ElectrumXBTE will occasionally connect to and verify its
...
.. envvar:: PEER_ANNOUNCE
  ...
  If not defined, or non-empty, ElectrumXBTE will announce itself to
  peers.

  If peer discovery is disabled this environment variable has no
  effect, because ElectrumXBTE only announces itself to peers when doing
...
.. envvar:: TOR_PROXY_PORT
  ...
  The port on which the Tor proxy is running.  If not set, ElectrumXBTE
...
.. _lib/coins.py: https://github.com/spesmilo/electrumxbte/blob/master/electrumxbte/lib/coins.py
