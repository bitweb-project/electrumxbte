...
**ElectrumXBTE** should run on any flavour of unix.  I have run it
...
Package          Notes
================ ========================
Python3          ElectrumXBTE uses asyncio.  Python version >= 3.7 is
                 **required**.
...
While not a requirement for running ElectrumXBTE, it is intended to be
...
When building the database from the genesis block, ElectrumXBTE has to
...
final size of the leveldb database, and other ElectrumXBTE file metadata
...
+ `plyvel <https://plyvel.readthedocs.io/en/latest/installation.html>`_ for LevelDB.
  ...
  Included as part of a regular pip or ``setup.py`` installation of ElectrumXBTE.
+ `python-rocksdb <https://pypi.python.org/pypi/python-rocksdb>`_ for RocksDB

  ``pip3 install python-rocksdb`` or use the rocksdb extra install option to ElectrumXBTE.
...
Check out the code from Github::
    ...
    git clone https://github.com/spesmilo/electrumxbte.git
    cd electrumxbte
...
under one account which I have called ``electrumxbte``.

Next create a directory where the database will be stored and make it
writeable by the ``electrumxbte`` account.  I recommend this directory
live on an SSD::

    mkdir /path/to/db_directory
    chown electrumxbte /path/to/db_directory


Process limits
--------------

You must ensure the ElectrumXBTE process has a large open file limit.
...
Note that setting the limit in your shell does *NOT* affect ElectrumXBTE
unless you are invoking ElectrumXBTE directly from your shell.  If you
are using :command:`systemd`, you need to set it in the
:file:`.service` file (see `contrib/systemd/electrumxbte.service`_).
...
logging process.  You can run :command:`svscan` under the *electrumxbte*
...
of::
    ...
    mkdir ~/service      # As electrumxbte if running svscan as that a/c
...
Suppose this directory is called :file:`scripts`, you might do::
    ...
    mkdir -p ~/scripts/electrumxbte

Then copy the all sample scripts from the ElectrumXBTE source tree there::

    cp -R /path/to/repo/electrumxbte/contrib/daemontools ~/scripts/electrumxbte
...
under :file:`env/` to your setup.  ElectrumXBTE server currently takes no
...
svscan is now waiting for services to be added to the directory::
    ...
    cd ~/service
    ln -s ~/scripts/electrumxbte electrumxbte
...
to setup ElectrumXBTE with systemd. Simply copy it to
...
:file:`/home/electrumxbte/electrumxbte`. If that differs on your system, you
need to change the unit file accordingly.

You need to set a few :ref:`environment variables <environment>` in
:file:`/etc/electrumxbte.conf`.

Now you can start ElectrumXBTE using :command:`systemctl`::
...
Once configured you may want to start ElectrumXBTE at boot::

    systemctl enable electrumxbte

.. Warning:: systemd is aggressive in forcibly shutting down
   processes.  Depending on your hardware, ElectrumXBTE can need several
...
`contrib/raspberrypi3/install_electrumxbte.sh`_.

See also `contrib/raspberrypi3/run_electrumxbte.sh`_ for an easy way to
configure and launch electrumxbte.
...
busy.  ElectrumXBTE uses Python's :mod:`asyncio` to prefill a cache of
...
size ElectrumXBTE uses; see :ref:`here <CACHE>` for caveats.
...
like so::
    ...
    svc -d ~/service/electrumxbte

ElectrumXBTE will note receipt of the signals in the logs, and ensure the
...
ElectrumXBTE uses the transaction functionality, with fsync enabled, of
...
if the ElectrumXBTE process if forcibly killed or there is loss of power.
...
Once the process has terminated, you can start it up again with::
    ...
    svc -u ~/service/electrumxbte

You can see the status of a running service with::

    svstat ~/service/electrumxbte
...
Here is typical log output on startup::
  ...
  INFO:BlockProcessor:software version: ElectrumXBTE 0.10.2
...
Starting with ElectrumBte 1.9, the client will learn and locally cache
...
You may choose to run electrumxbte on a different port than 50001
...
.. _`contrib/systemd/electrumxbte.service`: https://github.com/spesmilo/electrumxbte/blob/master/contrib/systemd/electrumxbte.service
...
.. _`contrib/raspberrypi3/install_electrumxbte.sh`: https://github.com/spesmilo/electrumxbte/blob/master/contrib/raspberrypi3/install_electrumxbte.sh
.. _`contrib/raspberrypi3/run_electrumxbte.sh`: https://github.com/spesmilo/electrumxbte/blob/master/contrib/raspberrypi3/run_electrumxbte.sh
